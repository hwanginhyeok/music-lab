...
| PIPE-F14 | 좀비 bot.py 재발방지 — 단일 인스턴스 가드 | 06-14 | `bot.py` 순수추가: `_acquire_single_instance_lock`(flock LOCK_EX·NB on `data/.music-bot.lock`) main() 최상단 호출 → 중복 인스턴스 경고+SystemExit(1). flock 자동해제(stale 없음). 2026-06-14 좀비(PID365) 토큰 가로채기 사고 재발방지. systemd-only 문서화. 실 subprocess 검증(2번째 exit1, 사망 후 3번째 획득). pytest 202. commit `a45288d`. 봇 재시작 활성화 완료 |
| PIPE-AUTO 마감 | 본체+운영 검증 전부 완료 → master 머지·배포 | 06-14 | **master `1b1b36b` 반영**(PR#1 squash, 30커밋→1). 실데이터 e2e(generate 우회, D-001 안티봇 재확인+suno_client Bug#1/#2 수정) → prefilter→[선택]→후처리(-14.00 LUFS 2-pass)→영상(1080p)→publish_approval 게이트(업로드 0). ADMIN_CHAT_ID 반영+캡차 알림 핑 `ok:true` 확인. 라이브봇 F02/F14 가동. DIFFICULTY D-001/D-008 + 메모리 기록. **남은 건 형님 액션 2건뿐**: ① run `86494cf1d566` '올려'(실 YouTube 업로드) ② Suno 안티봇 자동생성 대응(현재 프리필터부터 우회 운영) |
