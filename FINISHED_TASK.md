...
| # | 태스크 | 완료일 | 비고 |
|---|--------|--------|------|
| PIPE-F04 | 후처리 체인 파이프라인 (postprocess_v2.py 경량) | 04-24 | postprocess_v2.py 경량 버전 구현. Demucs/Matchering은 플래그 옵션으로 남김. 원래 스펙의 미니멀 경로 완성 |
| 4-1 | "봄이라고 부를게" YouTube 게시 | 04-23 | 싱글(fa7aabee) + 앨범(Suite 9트랙) 영상 업로드 완료. 설명문 작성 완료 |
